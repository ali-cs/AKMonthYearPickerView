...
private class MonthYearPickerView: UIPickerView, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    static var sharedInstance   = {
        return MonthYearPickerView(frame: CGRect(origin: CGPoint(x: 0, y: (Constants.AppFrameSettings.screenHeight - 256) / 2), size: CGSize(width: Constants.AppFrameSettings.screenWidth, height: 216)))
    }()
